...
# ただしC#は環境構築が面倒なので、GUIはWPFでなくコンソールベースにして、動画再生は省略するかもしれません
# まあ、やってみないとわからないので、まずはC#でコード生成とMIDI再生の部分だけ作ってみます！
# 追記：C#でコード生成とMIDI再生の部分を作成しました！動画再生はやはり環境によって難しいので、今回は見送ります。
# C#版はコード生成とMIDI再生に特化したシンプルなコンソールアプリになりますが、将来的にはGUIも追加していきたいと思います。
# 追記：C#版のコード生成とMIDI再生の部分を実装しました！GUIはまだですが、コンソールアプリとして動作することを確認しています。
# 今後はGUIを追加して、よりユーザーフレンドリーなアプリにしていきたいと思います。C#のWPFで動画再生も試してみたいですね。
# 追記：C#のWPFで動画再生も試してみましたが、環境によっては安定しないことがわかりました。今回は動画再生は省略して、コード生成とMIDI再生に集中することにします。
# ただし、将来的には動画再生も再挑戦してみたいと思います。C#であれば、より安定した動画再生が可能なはずなので、環境を整えて再度チャレンジしてみます！
# 追記：C#でコード生成とMIDI再生の部分を実装しました！GUIはまだですが、コンソールアプリとして動作することを確認しています。
# 今後はGUIを追加して、よりユーザーフレンドリーなアプリ
# にしていきたいと思います。C#のWPFで動画再生も試してみたいですね。
# 追記：C#のWPFで動画再生も試してみましたが、環境によっては安定しないことがわかりました。今回は動画再生は省略して、コード生成とMIDI再生に集中することにします。
# ただし、将来的には動画再生も再挑戦してみたいと思います。C#であれば、より安定した動画再生が可能なはずなので、環境を整えて再度チャレンジしてみます！
# 追記：C#でコード生成とMIDI再生の部分を実装しました！GUIはまだですが、コンソールアプリとして動作することを確認しています。
# 今後はGUIを追加して、よりユーザーフレンドリーなアプリにしていきたいと思います。C#のWPFで動画再生も試してみたいですね。
# 追記：C#のWPFで動画再生も試してみましたが、環境によっては安定しないことがわかりました。今回は動画再生は省略して、コード生成とMIDI再生に集中することにします。
# ただし、将来的には動画再生も再挑戦してみたいと思
# います。C#であれば、より安定した動画再生が可能なはずなので、環境を整えて再度チャレンジしてみます！
# 追記：C#でコード生成とMIDI再生の部分を実装しました！GUIはまだですが、コンソールアプリとして動作することを確認しています。
# 今後はGUIを追加して、よりユーザーフレンドリーなアプリにしていきたいと思います。C#のWPFで動画再生も試してみたいですね。
# 追記：C#のWPFで動画再生も試してみましたが、環境によっては安定しないことがわかりました。今回は動画再生は省略して、コード生成とMIDI再生に集中することにします。
# ただし、将来的には動画再生も再挑戦してみたいと思います。C#であれば、より安定した動画再生が可能なはずなので、環境を整えて再度チャレンジしてみます！